...
/**
 * 填写积分日志
 * @param array $data 数组变量
 * @param userId  用户id
 * @param sup  变动积分 
 * @param state 积分变动状态1-2+
 * @param supFrom 积分来源
 * @param fromInfo 来源明细
 * @param caozuoid 操作者id
 */
 function publicintegrallog($data){
      $supplier_log_model = M('supplier_log');
      $user_model = M('user');
      $da['userId'] = $data['userId'];
      //原积分
      $user_info = $user_model->where('id=%d',$da['userId'])->find();
      $da['oldSup'] = $user_info['integral'] + $user_info['otherintegral'];
      //导入积分
      $da['sup'] = $data['sup'];
      $da['state'] = $data['state'];
      //变动后积分
      //$da['newsSup']= $da['oldSup'] $da['state']==1?-:2+ $da['sup'];
      if ($da['state']==1) {
        $da['newsSup'] =  $da['oldSup'] - $da['sup'];
      }
      if ($da['state']==2) {
        $da['newsSup'] =  $da['oldSup'] + $da['sup'];
      }
      $da['regTime']=time();
      $da['supFrom']=$data['supFrom'];
      $da['fromInfo']=$data['fromInfo'];
      if($data['caozuoid']){
        $da['caozuoid']=$data['caozuoid'];
      }
      $supplier_log_model->add($da);
    }